...
- **群聊暂停** — `/pause` `/resume` 即时执行，免打扰期间消息仍落盘。WebUI tab 管理
- **定时提醒** — 一次性定时任务，LLM 自主计算时间戳，私聊发送，前端可查看
- **QQ 空间发帖** — 管理员触发 + 每日自动发帖（有日记才发），主 LLM 写 + 独立 LLM 把关
- **管理员代传话** — 延迟发送+自由措辞+语音表情包，WebUI tab 可管理
- **表情包收藏** — 自主收藏+本地落盘+向量搜索，独立通道发送，WebUI tab 可浏览/删除
- **情绪系统** — 5 种情绪+百分比，平滑过渡，温度联动，防提示注入
- **群聊免打扰** — 被吵醒生气自动暂停群到明早8点，WebUI tab 可管理
- **主动消息暂停** — LLM 自主判断暂停，系统计算时间，WebUI 可管理
