    ...
    async def _save():
        try:
            ...
            if r:
                ...
                handler._note_for_llm(conv_key, f"（系统提示：你刚才想存的表情包已经收藏过了，不用重复存，也不用刻意提存过了。）")
...
class MessageHandler:
    ...
    async def _handle_impl(
        ...
        json_prompt += (
            ...
            "- emotion: 你当前的情绪（只能有一种）。可选：开心/生气/难过/撒娇/平静。情绪变了就要填——比如被安慰好了不再生气了，填\"撒娇\"或\"开心\"告诉系统。没变留null。\n"
            "- save_sticker: 收藏对方发的表情包。你觉得可爱、有趣、鲨鱼相关、啊呜风格的都可以存——如果你在回复里夸它了，顺手存下来。但看不懂的、奇怪的、恶心的不要存。已收藏的会在下面列表里，看到重复的不用再存，也不用每次都特意提「存过了」。格式: {\"url\":\"图片URL\",\"description\":\"描述\",\"tags\":[\"标签\"]}，不喜欢就null。\n"
        ...
        # 睡眠时段注入群聊免打扰提示
        if mood.sleep_state in ("sleeping", "sleepy") and is_group:
            json_prompt += "\n- mute_groups: 你正在睡觉被群里消息吵醒了。基本都会填true开启免打扰暂停本群到明早8点，让你继续睡。开了的话在reply里自然提一嘴就行，比如「困了，先免打扰到明早哦」。只有极少数情况例外——比如很想接着聊可以留null。不代表生气，只是想继续睡。只有对方故意找你/叫你（@鱼、戳一戳、刷屏喊鱼）才在emotion里填生气。"
        ...
        if is_sender_admin and not is_group:
            ...
            messages.append({"role": "system", "content": (
                f"转达消息用 relay。可转达: {role_list}。to_role 必须严格匹配以上角色名。content 就是你要发给对方的话。可以提是谁让转达的，也可以完全以自己的口吻说。传话者让你多说就多说几句，不用刻意收短。看情况自然决定。\n"
            ...
            if send_s and isinstance(send_s, str) and send_s.strip():
                ...
                if matches:
                    ...
                    if path and path.exists():
                        ...
                        _group_id = group_id
                        _bot = self.bot_qq
                        _delay = len(self._split_reply(reply_text)) * 3.0 + 2.0
    ...
    async def _should_record_memory(self, mem: dict, text: str) -> bool:
        ...
        try:
            ...
            prompt = (
                "你是记忆过滤器。判断这条信息是否值得记住。\n"
                "值得记录：反映身份背景、重要经历、性格、约定、情感互动、有趣的小事。\n"
                "不值得记录：纯寒暄（早/晚安）、完全无信息量的语气词、重复已有记忆的内容。\n"
                "放宽一点——稍微有点意思就可以记，不用特别重大。\n"
                "只输出 YES 或 NO。"
            )
            payload = {
                "model": llm.model, "messages": [
