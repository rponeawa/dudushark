...
class MessageHandler:
    ...
    async def _should_skip_group(self, text: str, group_id: str = "", mentioned: bool = False) -> bool:
        ...
        try:
            ...
            prompt = (
                f"{persona_brief}\n\n"
                "你是过滤器。鱼在群里大部分时候安静，判断她是否应该回复。\n"
                "回复 YES：消息明显跟鱼有关（@鱼/接鱼的话）、在认真求助需要帮忙、话题正是鲨鱼/海洋/睡觉/软绵绵/科技/新奇事物让鱼眼睛发亮、鱼明显被cue到了。\n"
                "回复 NO：日常闲聊寒暄、路人对话、话题鱼完全没感觉、不确定跟鱼有没有关系。\n"
                "鱼的回复应该稀缺但不是零。遇到真正让她感兴趣或需要帮助的人，不要犹豫。\n"
                f"{at_note}\n"
                "只输出 YES 或 NO。"
            )
            payload = {
